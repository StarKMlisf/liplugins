...
export const works: WorkEntry[] = [
  ...
  },
  {
    ...
    summary: '属性界面 1-6 行自定义与槽位保留，监守者宠物主人保护、WG 区域开关与重召恢复跟随，捕捉品质、性格与闪光，持久化成长快照，以及骑乘、命名和自定义名牌。',
    description:
      'LiPet 0.29.6 修复属性界面小于 5 行时强制改行数及仓库按钮位置的问题，保留管理员布局。接管监守者宠物 AI，停用原版黑暗与声波，避免追打主人；野生监守者不变。保留 WG 召唤/攻击旗标，重新召唤恢复跟随；MM 禁刷怪区需管理员配置 WG 插件生成豁免。保留主人保护：禁止主人仇恨目标，双阶段拦截伤害并识别间接来源；保留追击绕障、卡住恢复和附近敌对怪物主动索敌，默认 8 格，坐下、骑乘及返回时不抢占控制。捕捉宠物支持品质、性格和闪光，已捕捉特性独立存档，旧宠物保持原数值。保留仓库、详情、名牌、PAPI、双商城、背包、等级模型与死亡扣级。材料进化及探索派遣为后续批次。',
