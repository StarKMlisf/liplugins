...
export const works: WorkEntry[] = [
  ...
  },
  {
    ...
    java: 'Java 21',
    availability: '免费',
    dependencies: ['PacketEvents 2.13.0+（必需）', 'PlaceholderAPI（可选）', 'CustomCrops（可选）', 'Dominion（可选）', 'Residence + 匹配的 CMILib（可选）', 'MythicMobs + ModelEngine（模型展示可选）'],
    features: [
      '四季与二十四节气',
      '客户端群系季节染色',
  ...
  },
  {
    ...
    summary: '监守者持续跟随与追击、宽体绕障和跳跃修复、召唤成功后显示名牌，停服保存、残留攻击隔离与嘎枝主人保护、重启残留清理与归属提示修正、鱼类水中跟随与骑乘升降、原生宠物体型缩放与等级倍率，属性界面 1-6 行自定义，监守者宠物主人保护、WG 区域开关与重召恢复跟随，捕捉品质、性格与闪光，持久化成长快照，以及骑乘、命名和自定义名牌。',
    description:
      'LiPet 0.29.11 修复监守者移动推进间隔、宽体寻路边角与跳跃中断，保留原版 Brain 关闭和主人保护。召唤成功注册后才显示名牌，失败及召回后的延迟任务不再重新创建名牌；不覆盖管理员配置，也不修改跟随阈值。保留停服保存修复，隔离无会话宠物伤害并正确清除嘎枝主人仇恨。保留重启残留与归属提示修复，加载后延迟核对并保护未完成召唤，不改存档主人。保留鱼类水中跟随与骑乘升降修复，AUTO 识别原版鱼并支持主动升降，新增 SWIMMING 模式。保留原生 SCALE 宠物体型、碰撞箱同步，支持公共及单宠等级倍率，默认关闭并跳过外部模型。属性界面继续支持 1-6 行并保留管理员布局。接管监守者宠物 AI，停用原版黑暗与声波，避免追打主人；野生监守者不变。保留 WG 召唤/攻击旗标，重新召唤恢复跟随；MM 禁刷怪区需管理员配置 WG 插件生成豁免。保留主人保护：禁止主人仇恨目标，双阶段拦截伤害并识别间接来源；保留追击绕障、卡住恢复和附近敌对怪物主动索敌，默认 8 格，坐下、骑乘及返回时不抢占控制。捕捉宠物支持品质、性格和闪光，已捕捉特性独立存档，旧宠物保持原数值。保留仓库、详情、名牌、PAPI、双商城、背包、等级模型与死亡扣级。材料进化及探索派遣为后续批次。',
    version: '0.29.11-SNAPSHOT',
    status: '持续维护',
    platform: 'Paper / Folia',
    minecraft: '1.21.11 · 26.1.2 · 26.2',
    java: 'Java 21+',
    ...
    features: [
      ...
      'Folia 新实体等待自己的下一 tick 完成召唤确认，避免 isValid 瞬时误回滚',
      'Folia 26.2 验证带外观宠物召唤、跨 1024 格认主与收回',
      '玩家重进与区块载入按持久化 UUID 恢复原版驯服主人',
      '全局喂食冷却支持 0-60 秒、热重载、失败撤销与不扣物品',
      '属性加点 revision 冲突自动对账并有限重试，避免偶发失败',
      '力量负责攻击与暴击伤害，敏捷负责暴击率、速度与闪避',
      '属性 GUI 动作兼容大小写，错误格式与点数不足提供中文提示',
      'Residence 局部晴天保护 LiPet 烈焰人，保留普通生物和真实泡水伤害',
      '连续属性加点使用本次已保存档案刷新，避免界面显示旧值',
      '旋风人风弹喂食防止原版发射与重复消耗，双手回退保护',
      '捕捉幼体固定年龄、区块恢复校正与主人关系保持',
      '快乐恶魂专用飞行跟随、追敌与大型实体高度目标',
      '快乐恶魂骑乘身体朝向与玩家实时视角同步',
      'Paper 26.2 的 87 种默认原版宠物逐项检查专属食物',
      '原版有食物标签时使用其实际接受的物品，狼猫模板同步修正',
      '全部官方默认食物每次 4 点经验，低于默认最弱战斗奖励',
      '旧服食物数值保持不变，可按经济配置手动调整',
      'Folia 名牌刷新先核对实体区域所有权，迁区临界点下一 tick 重试',
      '名牌最终失败保留当前状态，不在异常分支重复访问乘客实体',
      '全部默认可捕捉原版生物使用专属食物，烈焰人默认使用烈焰粉',
      '战斗经验按宠物合并保存，多人刷级保持操作顺序',
      'revision 冲突重读重试，关服等待经验批次避免等级回退',
      '经验保存失败向控制台与玩家明确提示',
      '技能书持久预扣、串行升级与失败、掉线、重启退款',
      '敏捷与速度技能统一作用于实体、跟随、战斗、GUI 与 PAPI',
  ...
  },
  {
    ...
    java: 'Java 21 / 25',
    availability: '免费',
    dependencies: ['无强制前置', 'MythicMobs / Citizens / PlaceholderAPI（可选）', 'Vault / PacketEvents / 队伍插件（按功能）'],
    features: [
      '统一 /lidungeon 与完整中文分页帮助',
      '经典、程序化、持续型地牢创建与编辑',
