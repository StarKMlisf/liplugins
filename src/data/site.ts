...
export const works: WorkEntry[] = [
  ...
  },
  {
    ...
    java: 'Java 21',
    availability: '免费',
    dependencies: ['PacketEvents 2.13.0+（必需）', 'PlaceholderAPI（可选）', 'CustomCrops（可选）', 'Dominion（可选）', 'Residence + 匹配的 CMILib（可选）'],
    features: [
      '四季与二十四节气',
      '客户端群系季节染色',
  ...
  },
  {
    ...
    summary: '修复道具商城提示与发放退款、飞行速度和骑乘疾跑加速，并统一命令、GUI、信号棒的可靠回收反馈。',
    description:
      'LiPet 是面向群组服的 Bukkit 宠物框架，采用 pet-types.yml 公共默认值与宠物目录单宠差异配置；默认捕捉规则允许的全部原版生物会生成中文独立 YML，MCPets 单宠文件可直接放入中文子目录并通过 MythicMobs 生成自定义实体。0.27.9 将道具商城余额不足提示与宠物商城分开，发放任务退休或失败会结束交易并退款；宠物速度同步实体支持的 FLYING_SPEED，飞行与地面骑乘按可配置倍率响应疾跑键；命令、管理 GUI 和信号棒共用可观察的回收反馈，超时会提示正在保存。0.27.8 的召唤队列恢复以及 0.27.7 的骑乘名牌全员隐藏、下马恢复和逐 tick 实时视角转向继续保留。插件同时提供三种商城货币名称热重载、旧原版类型 ID 兼容、每宠 MythicMobs 等级技能、管理页原路返回、“我的宠物”列表、仓库四键、SQLite/MySQL 热切换、双排行榜、PlayerPoints/Vault 商城、属性成长、MEG 等级模型、原版捕捉随机属性、真实捕捉球商店、捕捉仪式、完全自定义 GUI、离线及全服宠物管理、骑乘、战斗导航、外部模型、CraftEngine 完整物品挂钩与可启停的 CE 捕捉球/食物案例，以及扩展 API。',
    version: '0.27.9-SNAPSHOT',
    status: '持续维护',
    platform: 'Paper / Folia',
    minecraft: '1.21.11 · 26.1.2 · 26.2',
    java: 'Java 21+',
    ...
    features: [
      '道具商城独立余额提示，发放任务退休或失败时全额退款',
      '飞行属性与骑乘疾跑同步加速，倍率可按宠物配置',
      '命令、管理 GUI 与信号棒统一回收反馈和等待提示',
      ...
      '宠物币、Vault 金币与 PlayerPoints 点券名称独立配置',
      '名称成功重载后重开双商城生效，失败保留旧名',
      'MEG 模型按等级门槛切换与失败恢复',
      '原版捕捉四属性随机并持久保存',
      ...
      'pet-types.yml 公共默认值与单宠差异深层覆盖',
      'MCPets 单宠 YML 直接读取与 MythicMobs 实体生成',
      '主菜单“我的宠物”与宠物列表入口',
      '召唤上限专用中文提示与旧语言迁移',
      '中文属性成长、喂养与升级',
      '默认关闭且带完整中文注释的 CE 食物案例',
      '每宠独立 MythicMobs 技能、等级解锁与 power 成长',
      'MM 失败退避、逐技能日志限频与规则状态统计',
      '捕捉仪式、商城与宠物币奖励',
      '默认关闭且可直接修改的 CE 捕捉球案例',
      '跟随、坐下、召回与骑乘',
      '飞行宠物安全回传时名牌即时重建',
      '骑乘名牌隐藏可配置、Boss 条独立开关与末影龙飞行控制',
      '战斗 AI、死亡冷却与安全回传',
      '54 格背包与百分百自定义 GUI',
      '管理页属性/成长面板保留上下文并返回同一只宠物',
      '仓库四键操作、绿色潜行提示与独立红色永久删除警告',
      '旧“宠物类型”目录安全迁移为中文“宠物”目录',
      '数据库热切换与跨服租约',
