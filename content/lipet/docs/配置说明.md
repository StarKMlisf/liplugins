...
`config.yml` 新增 `death-penalty.enabled`（布尔，默认 true）、`levels-lost`（整数 0-10000，默认 1）、`minimum-level`（整数 1-10000，默认 1）。修改后 `/lipet reload` 生效。实际掉级会清空当前级经验并扣回相应属性点，见 [完整配置示例和边界](../changelog-2026-09-06-death-penalty/)。
...
feeding:
  ...
  # 设为 0 可关闭；修改后执行 /lipet reload 生效。
  cooldown-seconds: 1.0
```

冷却提示位于 `messages.yml -> pet-feed-cooldown`，支持 `<seconds>` 剩余秒数变量。旧配置只补缺失节点和中文注释，不覆盖管理员已有值。
## 0.27.14 原版专属食物

新安装会按 Paper 26.2 原版食物标签生成专属物品。主要修正为：`CAMEL_HUSK -> RABBIT_FOOT`、`PIGLIN/PIGLIN_BRUTE -> COOKED_PORKCHOP`、`SULFUR_CUBE -> SLIME_BALL`、`WOLF -> COOKED_BEEF`、`ZOMBIE_HORSE -> RED_MUSHROOM`、`ZOMBIE_NAUTILUS -> PUFFERFISH`。狼模板用熟牛肉和熟鸡肉；猫模板用生鳕鱼和生鲑鱼。

旧服的 `growth.foods` 被视为管理员配置，不会自动改名或删除。请备份对应宠物文件，手动替换节点并执行 `/lipet reload`。完整表格见 [0.27.14 更新日志](../changelog-2026-09-04-vanilla-pet-food-alignment/)。

## 0.27.13 默认喂食经验

官方普通食物、CraftEngine 案例、狼猫模板、原版商城模板和新生成原版宠物的 `growth.foods.*.experience` 默认统一为 `4`。默认最弱目标的战斗经验约为 `6`，20 点生命目标为 `15`。升级不覆盖旧服已有食物数值；需要采用本次平衡时手动改为 `4` 并执行 `/lipet reload`。

## 0.27.11 原版宠物食物与失败提示

官方原版宠物文件完全缺少 `growth.foods` 时会按物种补全。例如烈焰人生成启用的 `BLAZE_POWDER`，并把继承的 `COOKED_BEEF` 设为 `enabled: false`。已有 `growth.foods` 和自定义宠物不修改。

`messages.yml` 会补入以下根节点，现有自定义文本保留：

```yaml
# 战斗经验未成功写入数据库时提示在线主人。
pet-combat-experience-save-failed: "<red>宠物战斗经验暂时无法保存，请联系管理员检查数据库。</red>"
```
...
| 文件 | 用途 |
| --- | --- |
| `config.yml` | 存储、服务器 ID、群组、依赖下载、喂食冷却、内置货币与三种货币显示名称 |
...
`0.27.19` 新增 `pet-attribute-no-points` 与 `pet-attribute-invalid-action`。属性按钮继续使用 `attribute:<pet_id>:STRENGTH`，属性键不区分大小写；错误 UUID、缺少参数和未知属性会被安全拒绝。
...
- 官方默认 `experience` 为 `4`，低于默认战斗经验；自定义食物仍可填写其他非负数。
