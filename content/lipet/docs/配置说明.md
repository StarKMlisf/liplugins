...
`warehouse.pet-item.actions` 默认提供四种独立操作：`LEFT` 召唤所选宠物、`RIGHT` 查看属性与管理、`SHIFT_LEFT` 收回当前宠物、`SHIFT_RIGHT` 打开放生二次确认。四项均可自由更换或留空禁用；放生入口只打开确认页，不会单击直接删除。默认 Lore 同时带有“永久删除全部数据，无法恢复”的中文警告，只有官方旧默认 Lore、注释与动作会安全迁移，服主自定义内容不会被覆盖。
