...
目录中可放多个 `.yml` 或 `.yaml`，默认递归读取子目录。目录名、文件名和子目录名均可使用中文；稳定类型 ID 仍写在 `types` 下。新服会生成 `宠物类型/狼.yml` 与 `宠物类型/猫.yml`。旧服仅在目标目录尚不存在时自动按类型拆分旧 `types`，已有目录和自定义内容不会被覆盖。
...
技能放在对应宠物类型节点内，例如 `宠物类型/狼.yml` 的 `types.wolf`：
