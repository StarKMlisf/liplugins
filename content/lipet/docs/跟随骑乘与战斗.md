...
玩家未手动收回宠物便退出时，如果重进时活动实体仍存在，LiPet 会在宠物实体所属线程按档案中的主人 UUID 恢复狼、猫等原版驯服关系。区块重新载入和热重载使用同一恢复路径。鹦鹉继续不绑定原版主人，避免再次进入肩部数据。
## 0.27.15 快乐恶魂移动修复

成年快乐恶魂使用恶魂飞行控制器，但通用 Pathfinder 可能接受一条它无法稳定执行的地面路径。新版跟随与战斗会先停止快乐恶魂的旧路径，再由飞行速度接管；其他宠物仍保留原有寻路策略。

成年实体高 4 格，跟随与召回脚底目标改为主人上方 3 格，减少贴地和碰撞。骑乘任务还会同步快乐恶魂的身体 yaw，原地转向、持续前进后转头和多层模型座位继续每 tick 使用玩家当前视角。

## 0.27.12 Folia 名牌跨区安全刷新

宠物跨区域移动时，名牌刷新先检查实体是否属于当前区域线程；处于迁区临界点则交给宠物实体调度器在下一 tick 重试。异常分支不再调用 `getPassengers()` 或继续修改实体，因此不会把第二次 owning-region 异常抛出实体任务。

上马事件会立即记录隐藏状态，下马后一 tick 按最终乘客树校正；行为循环把已确认的骑乘结果直接传给名牌服务，仍兼容多层模型座位。已有 `nameplate.hide-while-ridden` 设置及下马恢复行为不变。

## 0.27.11 战斗经验结算

战斗宠物连续击杀时，同一只宠物在数据库写入期间积压的经验会合并为下一批保存；不同玩家各自保持操作顺序。回收和关服保存排在经验批次之后，revision 冲突会读取最新数据库记录后重试。保存失败会写控制台警告并通知在线主人，便于区分宠物币已到账与经验未保存。

## 0.27.10 最终移动速度

地面与飞行实体属性、跟随导航和战斗追击统一读取“基础速度 + 敏捷 + 速度技能”。成长加点后的属性刷新会重新应用技能速度，避免迅捷技能被覆盖；GUI 与 `%lipet_active_speed%` 也显示同一结果，最多保留三位小数。
...
召唤时会为宠物安装高优先级移动与目标控制 Goal，原版闲逛、逃跑和自动仇恨不能再反向覆盖插件路径。全部地面宠物都由服务端原生 Pathfinder 绕开方块：跟随每 10 tick 更新落点，战斗每 10 tick 刷新路径、每 2 tick 检查攻击距离；路径不可达时会停止旧路径，不会把宠物直线拖过障碍。快乐恶魂因成年原生导航与飞行控制器组合不稳定，固定使用平滑飞行速度；其他飞行类在运行端确实没有可用路径时使用同一兜底。骑乘时快乐恶魂额外同步身体朝向；实际伤害仍由统一属性与攻击冷却计算。
