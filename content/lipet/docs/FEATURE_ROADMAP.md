...
已完成骑乘迈步/朝向、命名规则、多行名牌，以及捕捉品质、性格与闪光持久化。
材料进化和挖矿/伐木/采集派遣仍为后续批次，尚未包含在当前成品。
...
- 单宠召唤上限的独立中文反馈与官方旧语言安全迁移。
- 主菜单“我的宠物”入口与“打开宠物列表”语义化文案迁移。
- 仓库左键召唤、右键管理、潜行左键收回、潜行右键放生的四键独立动作与提示；潜行操作统一绿色，永久删除警告独立为红色段落。
- 管理页打开的完整属性与成长加点面板会保留宠物上下文，返回时回到同一只宠物的管理页。
- 支持中文命名目录、中文子目录和多个 YML 的宠物类型配置存储。
- 每个宠物类型可独立配置 MythicMobs 技能，按宠物等级解锁，并支持触发器、概率、独立冷却、失败退避与 `power` 成长。
...
MythicMobs 技能作为可选外部动作接入，不把 MM API 打入 LiPet。技能直接放在对应的 `宠物类型/*.yml` 中，支持用 `unlock-level` 控制解锁等级，并通过 `power-per-level` 让 MM 技能威力随宠物等级成长。冷却判断位于概率抽取前，定时触发一次扫描两种周期规则；失败技能保留配置冷却并有最短退避，状态指令可查看已载入类型数和规则数。
