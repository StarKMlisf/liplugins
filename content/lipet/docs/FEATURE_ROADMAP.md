...
- 每个宠物类型可独立配置 MythicMobs 技能，按宠物等级解锁，并支持触发器、概率、冷却与 `power` 成长。
...
MythicMobs 技能作为可选外部动作接入，不把 MM API 打入 LiPet。技能直接放在对应的 `宠物类型/*.yml` 中，支持用 `unlock-level` 控制解锁等级，并通过 `power-per-level` 让 MM 技能威力随宠物等级成长。
