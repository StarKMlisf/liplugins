...
> 当前风险：同一宠物背包尚无独占编辑会话。主人与管理员同时打开、关闭保存未完成又重新打开，可能造成覆盖或物品重复。暂勿多人同时编辑同一宠物背包；这与只读菜单的点击保护是两个问题，见 [逐项排查清单](../audit-2026-09-06-lifecycle/)。

> 本页从 LiPet `0.27.10-SNAPSHOT` 完整 Wiki 同步整理；需要连续查阅全部功能时，请返回 [LiPet 完整 Wiki](../home/)。

## 0.27.10 技能书稳定升级

技能书会先写入插件维护的持久预扣台账，再扣除一本并进入玩家串行队列。连续右键会逐本结算；失败、满级、宠物不存在、玩家掉线或服务器中途重启时会退还对应技能书。`pending-skillbook-refunds.yml` 是内部退款台账，请勿手动编辑。MySQL 还会在事务内锁定宠物与技能记录，避免群组服并发升级丢失等级。
...
`0.27.9` 起，信号棒回收、`/lipet store` 和管理 GUI 共用同一结果反馈。回收超过 100 tick 时只提示一次正在保存；数据库必须先确认 STORED，随后才移除实体和活动会话。保存失败会保留召唤状态，反馈发送异常会使用独立的 `pet-store-feedback-failed`，不会把已经成功的回收误报为保存失败。
