...
- `hide-while-ridden` 控制骑乘时隐藏双行名牌，`riding.hide-boss-bar-while-ridden` 控制凋零、末影龙等 Boss 血条；下坐骑后都会自动恢复。
...
`0.26.16+` 会识别末影龙翅膀、头部和身体部件，空手右键可骑乘，潜行空手右键可打开管理界面。玩家骑上宠物时会按配置自动隐藏名牌和 Boss 血条。
