...
在宠物仓库中，左键召唤所选宠物，右键打开属性与管理，潜行左键收回当前已召唤宠物，潜行右键进入永久放生二次确认。两条潜行操作默认使用绿色提示，红色 `⚠放生会永久删除全部数据` 会独立显示在操作说明下方；放生不会在第一次点击时直接执行。

从宠物管理界面点击“完整属性”或“成长与加点”后，属性面板底部会显示“返回管理界面”；点击后回到同一只宠物的管理页，不需要关闭菜单再潜行右键宠物。从仓库或指令进入属性面板时仍返回宠物仓库。

同一时间只能召唤一只宠物。已有宠物正在跟随时，再使用 `/lipet call` 或仓库召唤另一只会提示“已达最大召唤数量，请将已召唤的休息后再召唤！”。先执行 `/lipet store` 或使用仓库收回功能让当前宠物休息，再召唤新的宠物。

`0.27.8` 起，命令与仓库共用召唤提示。操作未完成超过 100 tick（正常 TPS 下约 5 秒）时才提示一次“正在召唤”，请等待结果，无需重复点击；服主可以关闭这条等待提示。看到“召唤结果提示异常”时，应先检查宠物是否已经出现，再联系管理员查看日志，这条消息不代表宠物一定没有生成。旧版偶发无反应且重载无效的问题需要服主换用新 Jar 后重启，详见 [0.27.8 召唤无响应修复](更新日志-2026-09-01-召唤无响应修复.md)。

服主为某种宠物配置 MythicMobs 等级技能后，玩家无需使用额外指令或技能书；宠物达到要求等级便会自动解锁。技能可在主人或宠物战斗、主人交互及定时行为中触发，具体等级、概率和冷却由服务器配置决定。
