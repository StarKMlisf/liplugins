...
`0.26.11-SNAPSHOT` 的 80 项自动测试已在默认兼容构建与 Paper 26.2 Profile 全部通过。同一 Java 21 成品 Jar 已在 Paper 26.1.2 Build 70 与 Paper 26.2 Build 111 使用 Java 25 完成真实启动、`/lipet status`、配置补全和安全关闭；26.1.2 还验证了 `shop.yml` 从 `enabled: false` 热重载为 `true` 后，帮助页商城入口立即恢复。管理上下文所有权、实时权限、指定宠物 UUID 与管理员背包编辑者隔离均有回归测试覆盖。
