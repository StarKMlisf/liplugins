...
- 仓库左键或蹲下左键宠物：召回伙伴；右键宠物：查看等级、经验、属性和食物；蹲下右键：进入永久放生二次确认。
...
输出：`target/LiPet-0.26.22-SNAPSHOT.jar`
