...
build.107 起可通过 `sell-pricing` 为不同权限设置出售倍率，例如普通玩家八折、VIP 九折、SVIP 原价回收。只影响转换菜单与一键出售，不改变购买、自动补充、退款或转账。

功能默认关闭；多个规则命中时取 `priority` 最大的一条，同级取配置靠前者，不叠乘。配置、授权和 GUI 变量示例见[出售权限折扣与倍率](./出售权限倍率.md)。
