...
2. 将 `LiSkin-1.9.6.jar` 放入 `plugins`。
...
LiSkin 1.9.6 可以在 A 点常驻一个原生玩家人偶 NPC。玩家蹲下并用主手右键 NPC 后，会直接看到全部皮肤的分页菜单；点击服装只启动试衣，不会购买、解锁或修改玩家手中的物品。
...
观看模型时想换另一套，不需要重新输入 `/liskin fitting`。直接蹲下并用主手右键任意位置，全部服装菜单会再次打开；选择新皮肤后继续在同一个 A/B 点观看，最终退出仍返回第一次进入试衣间前的位置。
