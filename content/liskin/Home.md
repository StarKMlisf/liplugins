...
2. 将 `LiSkin-1.9.7.jar` 放入 `plugins`。
...
- `/liskin fitting` 或 `/liskin fitting menu`：打开顶部分类服装试衣菜单。
...
LiSkin 1.9.7 可以在 A 点常驻一个原生玩家人偶 NPC。玩家蹲下并用主手右键 NPC 后，会看到顶部两行固定分类按钮、下方排列对应皮肤的分页菜单；“全部”和 16 个装备/武器分类都能直接点击。点击服装只启动试衣，不会购买、解锁或修改玩家手中的物品。
...
开始试衣后，玩家会传送到 B 点并默认临时进入旁观模式，以隐藏手持物和生存界面，只观看 A 点生成的个人试穿人偶。人偶套用该玩家自己的皮肤并自动旋转，武器拿在主手，头饰、胸甲、护腿和靴子进入对应装备槽；背包等展示实体会跟随人偶一起旋转。

观看模型时想换另一套，不需要重新输入 `/liskin fitting`。直接蹲下并用主手右键任意位置，分类服装菜单会再次打开；选择新皮肤后继续在同一个 A/B 点观看。到期、退出、死亡、生成失败、重载或停服都会恢复第一次进入前的游戏模式，最终退出仍返回原位置。
